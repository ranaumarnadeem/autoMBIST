...
// slot B of a 2-memory subsystem, 5-bit address (32 words) x 8-bit data,
// 1 spare row, a fixed stuck-at-0 defect at (addr=20, bit=6) -- a DIFFERENT
// address/bit/polarity than slot A's defect, to prove the two memories'
// self-repair is genuinely independent, not coincidentally identical.
module sram_spares_intmem_b #(
    parameter integer ADDR_WIDTH     = 5,
    parameter integer DATA_WIDTH     = 8,
    parameter integer NUM_SPARE_ROWS = 1,
    parameter integer MEM_ADDR_WIDTH = $clog2((1 << ADDR_WIDTH) + NUM_SPARE_ROWS),
    parameter integer DEFECT_ADDR    = 20,
    parameter integer DEFECT_BIT     = 6,
