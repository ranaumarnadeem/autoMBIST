"""Standalone build/run script for the 2RW march algorithm testbench.

This does NOT go through generate_from_config() or the Jinja wrapper
templates -- it compiles the march_2rw RTL family directly against
march_2rw_tb_top.sv + sram_model_2rw.sv using cocotb's Python Runner API
(cocotb_tools.runner), so no hand-written Makefile is required. Mirrors
tests/hardware/run_march_1r1w_tb.py exactly.
...
SIM_BUILD_DIR = HW_TEST_DIR / "sim_build" / "march_2rw"
...
def main() -> int:
    ...
    sources = [
        RTL_DIR / "sram_model_2rw.sv",
        RTL_DIR / "march_2rw" / "march_2rw_algo.sv",
        RTL_DIR / "march_2rw" / "march_2rw_fsm.sv",
        RTL_DIR / "march_2rw" / "march_2rw_top.sv",
        HW_TEST_DIR / "march_2rw_tb_top.sv",
    ]
    ...
    print(f"\n=== march_2rw testbench summary: {total_tests} tests, "
