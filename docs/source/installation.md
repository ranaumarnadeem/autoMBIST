...
the redundancy/repair work), last tagged `v1.1.2`, remains published on PyPI
from before this project moved to Nix as its packaging path. It isn't kept
up to date and isn't the recommended way to install autoMBIST. Versioning
restarted from `0.0.1` when Nix became the packaging path, so
`autombist --version` on a Nix checkout (`0.1.0` and up) is always lower
than the legacy PyPI release's `1.1.2` — go by installation method (Nix vs.
`pip install autombist`) if you need to be certain which one you have, and
see the
