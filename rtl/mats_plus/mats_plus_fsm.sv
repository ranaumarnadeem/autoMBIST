...
module mats_plus_fsm #(
    ...
    // MATS+ has 3 elements (phases 0-2); LAST_PHASE and phase_is_up are sized
    // for this algo's own phase count/direction sequence, NOT copy-pasted from
    // march_c_fsm's (which has 6 phases) -- see mats_plus_algo.sv's case table
    // for the direction each phase actually runs.
    localparam logic [2:0] LAST_PHASE = 3'd2;
    ...
    // Direction of a given phase: E0-E1 ascend, E2 descends (matches
    // mats_plus_algo.sv's own phase_dir_up assignments -- this lookahead
